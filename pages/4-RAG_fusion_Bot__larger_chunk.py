...
st.title("RAG-fusion Chatbot (larger chunk)")
st.write(
...
if "messages_2_larger" not in st.session_state:
    st.session_state.messages_2_larger = []

# Display chat messages from history on app rerun
for message in st.session_state.messages_2_larger:
...
if prompt := st.chat_input():
    ...
    st.session_state.messages_2_larger.append({"role": "user", "content": prompt})

    response = final_rag_chain.invoke({"question": prompt})
    st.chat_message("ai").write(response)
    st.session_state.messages_2_larger.append({"role": "ai", "content": response})
