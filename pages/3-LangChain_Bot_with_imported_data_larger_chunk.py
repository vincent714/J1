...
st.title("Chatbot with imported data  (larger chunk)")
st.write(
...
if "messages_larger" not in st.session_state:
    st.session_state.messages_larger = []

# Display chat messages from history on app rerun
for message in st.session_state.messages_larger:
...
if prompt := st.chat_input():
    ...
    st.session_state.messages_larger.append({"role": "user", "content": prompt})

    response = chain.invoke(prompt)
    st.chat_message("ai").write(response)
    st.session_state.messages_larger.append({"role": "ai", "content": response})
